...
if __name__ == '__main__':
    import time
    from _2_data_cleaning import clean_data_all

    data_read = pd.read_csv('Raw_data_lb_belfast_TERMINAL.csv')
    data_read['timestamp'] = pd.to_datetime(data_read.timestamp, format='%Y-%m-%d %H:%M')
    ...
    print('The percentage of values with an undefined speed =', round(speed[0] * 100 / data_2.shape[0], 3), '%')

    print('The total time for data_enrichment.py:', time.time() - start_time, 'seconds')

    data_2.to_csv('Processed_data_lb_belfast_TERMINAL.csv')
