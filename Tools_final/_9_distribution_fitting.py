...
#    p_2 = []
 ...
 #   p_2.append((scipy.stats.chisquare(observed_values, expected_values))[1])
 ...
 #   p_2.append((scipy.stats.chisquare(observed_values, expected_values))[1])
  ...
  #  p_2.append((scipy.stats.chisquare(observed_values, expected_values))[1])
    ...
    df_results_iat['p'] = p
 #   df_results_iat['p_2'] = p_2
 #    df_results_iat['chi'] = chi
    df_results_iat = df_results_iat.sort_values(by=['p'], ascending=False)
    # gek dat ascending hier werkt niet zoals verwacht, verwacht juist andersom
...
def st_distributions(df):
    ...
    plt.plot(x_0, fitted_data_beta, 'r-', label='Beta')
    plt.legend(title='Distributions')
...
if __name__ == '__main__':
    ...
    df_iat = iat_distributions(df)

    # Choose best distribution for service times
    df_st = st_distributions(df)
