...
from data_loading import adjust_rhdhv_data, sort_data_rows
...
def clean_duplicates(data):
    ...
    for row in data.itertuples():
        ...
        if (row.Index != 0) and row.lat == prev_row.lat and row.lon == prev_row.lon:
            prev_row = row
            drop_list.append(row.Index)
            continue
        # If the timestamp of the current row matches that of the previous row it is a duplicate.
        if row.timestamp == prev_row.timestamp and row.mmsi == prev_row.mmsi:
            drop_list.append(row.Index)
            continue
        prev_row = row
...
if __name__ == '__main__':
    ...
    data_read = pd.read_csv('Raw-dataset_SH_largeterminal_0101-0201.csv')
    data_input = sort_data_rows(adjust_rhdhv_data(data_read))
